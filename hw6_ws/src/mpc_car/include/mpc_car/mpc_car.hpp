...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    VectorG gc_;
    gc_ << v * phi * sin(phi), -v * phi * cos(phi), 0, -v * delta / (ll_ * cos(delta) * cos(delta));
    
    Ad_ = Ac_*dt_+Eigen::Matrix4d::Identity();
    Bd_ = Bc_*dt_;
    gd_ = gc_*dt_;
  ...
  inline void step(VectorX& state, const VectorU& input, const double dt) const {
    // Runge–Kutta
    // https://zhuanlan.zhihu.com/p/427409568?utm_oi=1216413085835370496
