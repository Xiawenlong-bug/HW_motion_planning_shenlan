    ...
    */
    ...
    for (int i = 0; i < matrix_eigenvalues.size(); i++)
    {
        if (matrix_eigenvalues(i).imag() == 0.0 && matrix_eigenvalues(i).real() > 0.0)
        {
            ...
            double optimal_J = Homeworktool::getJ(_start_position, _start_velocity, _target_position,t);
    ...
    ROS_INFO("FINDING THE FINAL OPTIMAL SOLUTION...");
    return optimal_cost;
}

double  Homeworktool::getJ(Eigen::Vector3d _start_position,Eigen::Vector3d _start_velocity,Eigen::Vector3d _target_position,double T){
    double px0 = _start_position(0);
    double py0 = _start_position(1);
    double pz0 = _start_position(2);
    double pxf = _target_position(0);
    ...
    return (T + (4.0 * vx0 * vx0 + 4.0 * vy0 * vy0 + 4.0 * vz0 * vz0) / T + \
