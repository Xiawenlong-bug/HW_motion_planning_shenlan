...
void execCallback(const ros::TimerEvent &e)
{
  ...
  case REPLAN_TRAJ:
  {
    ...
    cout<<"[INFO]start_pt:"<<start_pt<<"vel:"<<start_vel<<endl;
    bool success = trajGeneration();
    cout<<"[INFO]STILL THERE?"<<endl;
    if (success)
      changeState(EXEC_TRAJ, "STATE");
    else
      changeState(GEN_NEW_TRAJ, "STATE");
...
bool trajGeneration()
{
   ...
   *
   * **/
  ROS_WARN("Traj generation BEGIN");
  bool astar_success = _astar_path_finder->AstarGraphSearch(start_pt, target_pt);
  ...
  cout<<"grid_path size: "<< grid_path.size()<<endl;  
...
void trajOptimization(Eigen::MatrixXd path)
{
   ...
   *
   * **/
  ROS_WARN("TIME ALLOCATION BEGIN");
  _polyTime = timeAllocation(path);
  ROS_WARN("TIME ALLOCATION DONE");
  /**
   *
   ...
   *
   * **/
  ROS_WARN("POLYNOMIAL TRAJECTORY GENERATION BEGIN");
  ros::Time TIME1=ros::Time::now();
  _polyCoeff =
      _trajGene->PolyQPGeneration(_dev_order, path, vel, acc, _polyTime);
  ros::Time TIME2=ros::Time::now();
  ROS_WARN("POLYNOMIAL TRAJECTORY GENERATION DONE-----USE TIME: %f",(TIME2-TIME1).toSec());
  ...
  while (unsafe_segment != -1)
  {
    ...
    //count=segment+1
    repath.row(unsafe_segment+1) = (path.row(unsafe_segment) + path.row(unsafe_segment + 1)) / 2;
...
VectorXd timeAllocation(MatrixXd Path)
{
  ROS_WARN("ALLOCATION-----1");
  cout << "in function timeAllocation,path size is :" << Path.size() << endl;
  VectorXd time(Path.rows() - 1);
  ROS_WARN("ALLOCATION-----2");
  double max_acc_t = _Vel / _Acc;
  double max_d = 0.5 * _Acc * pow(max_acc_t, 2);
  int n = time.size();
  cout<<"n:"<<n<<endl;
  for (int i = 0; i < n; i++)
  {
    ROS_INFO("ENTER LOOP,%d", i);
    double d = (Path.row(i + 1) - Path.row(i)).norm();
    if (d <= 2 * max_d)
    {
      time(i) = 2 * sqrt(d / _Acc);
    }
    else
    {
...
int main(int argc, char **argv)
{
  ...
  nh.param("planning/dev_order", _dev_order, 4); // minimum snap
